...
test("rich Markdown is responsive and shared by bodies, comments, and previews", async ({ page, request }) => {
  ...
  for (const fixture of [
    ...
    await page.addInitScript((theme) => localStorage.setItem("nodebook-theme", theme), fixture.theme);
    ...
    await expect(body.getByRole("img", { name: "NodeBook image" })).toHaveCSS("max-width", "100%");
    ...
    await expect(page.locator(".comment .markdown").locator(".markdown-table-wrap")).toBeVisible();
  }
