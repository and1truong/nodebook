...
The browser planning workspace (`/calendar`) renders open issues' due dates (all-day) and scheduled instants (timed) in navigable day/week/month views, Sunday-first to match `DatePicker`. All arithmetic is civil-date math on `Date.UTC` — no local-time parsing, no timezone database — so month grids, week starts, and month navigation (with day-of-month clamping: Jan 31 + 1 month = Feb 28/29) are deterministic in any browser.

`GET /api/planning/calendar?start=YYYY-MM-DD&end=YYYY-MM-DD&tz=...` serves a **bounded, end-exclusive** range: `start`/`end` must be valid civil dates, `end > start`, and the span is capped at 62 days (a six-week month grid plus slack). The repository query is indexed on `idx_issues_due`/`idx_issues_scheduled`; due dates compare as civil strings against `[start, end)`, while scheduled instants compare as UTC ISO strings against the timezone-derived instant bounds of the same civil window (`instantFromCivil`), so an instant's civil date in the viewer's timezone is in range exactly when its instant is — correct across DST transitions. The service expands each issue into separate `CalendarItemDto` entries per planning value (an issue with both due and scheduled dates yields two entries, ordered by date, kind, then issue number) and leaves `start_date` excluded, matching the legacy Upcoming semantics. `/upcoming` remains as a client-side compatibility redirect, and the Upcoming HTTP API plus MCP `get_upcoming` are unchanged for existing consumers.
...
- `test/integration` (Workers runtime via `@cloudflare/vitest-pool-workers`) — the full worker under `SELF.fetch` with real D1/R2/DO bindings: auth rejection, concurrent numbering, state transitions, cycle/duplicate prevention, late-resolving backlinks, FTS ranking/filters/rebuild, planning boundaries (including the calendar range/timezone/DST/dual-entry surface), reminder idempotency/locks/recalculation, attachment dedupe/ranges/GC, scheduled handlers, and the complete MCP surface including HTTP/MCP audit parity. Migrations are applied per test file from `test/integration/setup.ts`.
