...
### Sub-issues (GitHub-style lazy panel)
`GET /api/graph/:ref/sub-issues` returns **one hierarchy level per request**: the direct children of the ref as flat `SubIssueSummaryDto`s (id, number, title, status, parent_id, child_count, closed_child_count) ordered by number. Each row's `child_count`/`closed_child_count` describe that issue's own direct children, so expand controls and progress badges render without fetching descendants — the panel's completion progress is **closed direct children / total direct children** per node, matching GitHub's nested progress badges. The client loads the root level on mount, starts every branch collapsed, and fetches a branch's children on first expansion (`GET /api/graph/:ref/sub-issues` for that issue); successfully loaded branches are cached for the life of the page, failures stay local with a retry, and navigating to another issue resets the cache. Missing roots return 404; leaves return an empty array. The legacy `GET /api/graph/:ref/children` (flat, full `IssueDto`s) is retained for compatibility.
...
- **Issue detail layout (view mode).** `IssueDetailPage` renders a GitHub-style two-column layout for both `/issues/:ref` and `/wiki/:ref`: a full-width header (`issue-head`) keeps the number, title, status badge, and Complete/Close/Reopen/Edit actions; below it an `issue-layout` grid (`minmax(0, 1fr) 280px`, stacking below `min-width: 1200px`) holds the conversation column (`issue-main`: Markdown body, lazy `SubIssuesPanel`, Comments, History) and the `IssueSidebar` (`<aside aria-label="Issue details">`). The sidebar renders compact border-separated sections: properties (type, priority, labels, parent link, start/due/scheduled dates with overdue styling, recurrence rule, closed time — `None` where unset), attachments, reminders, relationships, and backlinks. `ReminderEditor` and `RelationshipsPanel` take an `embedded` prop that suppresses their outer card/heading and stacks full-width controls for the narrow rail; all selectors (`.uploader`, `.attachment-item`, `.reminder-form`, `.reminder`, `.rel-item`, backlink links) are unchanged. Edit/create forms stay single-column. AppShell widens its content limit to `1280px` only on issue/wiki detail routes (create excluded); lists and settings keep `980px`.
