...
| M5 | Planning views | Inbox/Today/Calendar/Overdue in owner timezone; `GET /api/planning/calendar` (bounded end-exclusive civil range, timezone-derived instant bounds, dual due/scheduled entries) powers the browser Calendar workspace, which opens in the `CALENDAR_DEFAULT_VIEW`-configured view (fallback week) and reschedules entries by drag-and-drop with optimistic persistence | `test/integration/planning.test.ts` (boundaries, ordering, timezone, DST, dual entries, invalid ranges, reminder recalc after a move), `test/unit/calendar.test.ts` (grids, navigation, clamping, patches), `test/unit/config.test.ts` (default-view resolution), e2e `calendar.spec.ts` (views, redirect, drag, keyboard, failures, mobile) |
...
- **The calendar default view is deployment configuration, not client state.** `CALENDAR_DEFAULT_VIEW` (`day`/`week`/`month`, missing or invalid values fall back to `week`) is resolved on the Worker and exposed through authenticated `/api/me` (`calendar_default_view`), so deployments switch the initial view without a rebuild. The client gates its first range fetch on this config (no incorrect-view flicker or duplicate requests) and keeps view switches session-local — they never overwrite the deployment default or persist.
- **Dragging reschedules through the existing issue PATCH.** Entries are draggable between dates (pointer/keyboard/touch via `@dnd-kit/core`, with a movement threshold so entry-link clicks still navigate; the day agenda offers a **Move date…** picker instead). A drop moves only the dragged field — due entries change `due_date`, scheduled entries change `scheduled_date` while preserving the viewer-local wall-clock time (DST-safe via `instantFromCivil`); same-date and invalid drops are no-ops. The client updates optimistically, reconciles with the PATCH response, blocks concurrent moves of one issue, and rolls back with a non-destructive alert on failure. Validation, audit history, recurrence anchors, and before-due reminder recalculation all run server-side unchanged.
